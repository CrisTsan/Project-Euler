...
void perm(string str, string before) {
    if (str.length()==1) {
        globCounter++;
        if (globCounter==1000000) cout << before << str <<endl;
        return;
    }
    for (int i=0; str.length()>i; i++) {
        string reduced,removed;
        removed = str[i];
        reduced =  str.c_str();
        reduced.erase (reduced.begin()+i);
        perm(reduced,before+removed);
    }
}
int main () {
    string initial = "0123456789", dumb="";
